    ...
    public partial class MantenedorUsuarios : System.Web.UI.Page
    {
        ...
        protected void btnRegistrar_Click(object sender, EventArgs e)
        {
            if(ddlEstado.SelectedValue != "")
            {
                if (ddlTipoUsuario.SelectedValue != "")
                {
                    try
                    {
                        Usuario usuario = new Usuario()
                        {
                            ...
                            Tipo_usuario = ddlTipoUsuario.SelectedValue,
                            Usuario_ = txtUsuario.Value
                        };
                        if(usuario.Read())
                        {
                        ...
                        {
                            string mensaje = "Usuario: "+usuario.Usuario_+" creado";
                            usuario.Create();
                            ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + mensaje + "');", true);
                        }
                    }
                ...
                {
                    string ex = "Debe Seleccionar Tipo de Usuario";
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + ex + "');", true);
                }
            }
            else
            {
                string ex = "Debe Seleccionar Estado";
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + ex + "');", true);
            }
        }
    }
